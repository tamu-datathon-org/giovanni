"use client"
import { Team } from "~/server/api/routers/team";

export const TopThreeComponent = ({ index, team, maxScore }: { index: number, team: Team, maxScore: number }) => {
    return (<li
        key={index}
...
export const AdjacentTeamsComponent = ({ index, competitor} : { index : number, competitor : Team, colorClass : string | undefined }) => {
    return (
        <li
            key={index}
        ...
        >
            ...
            <div
                className={`absolute bottom-0 right-0 top-0 flex h-full w-8 items-center justify-center rounded-lg z-0`}
            >
