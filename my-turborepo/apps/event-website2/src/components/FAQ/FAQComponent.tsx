...
export default function FAQComponent({ faqData }: FAQProps) {
    ...
    return (
        <div id="FAQ" className="max-w-4xl mx-auto p-2">
            <h2 className="w-full pb-4 text-center font-[myfont] text-3xl sm:text-4xl md:text-5xl mb-3">FAQ</h2>
            <div className="flex justify-center w-full">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
                    {faqData.map((faq, index) => (
                        <div
                            key={index}
                            className="relative rounded-2xl shadow-sm w-full overflow-hidden transition-all duration-300"
                        >
                            <button
