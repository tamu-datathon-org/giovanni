...
import {Form, FormControl, FormField, FormItem, FormLabel, FormMessage,} from "@vanni/ui/form";
...
export function ApplicationForm() {
    const {
        data: importedValues,
        isLoading
    } = api.application.getApplicationByEventName.useQuery({eventName: process.env.NEXT_PUBLIC_EVENT_NAME || ""}, {
        refetchOnWindowFocus: false, refetchOnReconnect: false, retry: 1, // Retry failed requests once
    },);

    const form = useForm<ApplicationSchema>({
        mode: "onSubmit", defaultValues: {
            resumeFile: null, liabilityWaiver: false,
        }, ...importedValues?.app, resetOptions: {
    ...
    const onSubmit: SubmitHandler<ApplicationSchema> = async (data) => {
        ...
        if (data.resumeFile) {
            await upload(data.resumeFile.name, data.resumeFile, {
                access: "public", contentType: "application/pdf", handleUploadUrl: "/api/resume",
            })
                .then((blob) => {
                ...
                .catch((error) => {
                    toast({
                        variant: "destructive", title: "Resume Failed to Upload", description: error.message,
                    });
                });
        } else {
        ...
        if (!blob_name || !blob_url) {
            toast({
                variant: "destructive", title: "Submission failed", description: "Resume file is required.",
            });
            return;
        }
        ...
        if (!importedValues) {
            const createApplicationData = {
                ...
                applicationData: {
                    ...data, gradYear: Number(data.gradYear),
                },
            };
            ...
            await createApplication.mutateAsync(createApplicationData, {
                onSuccess: () => {
                    ...
                    if (error instanceof TRPCClientError) {
                        toast({
                            variant: "destructive", title: "Submission failed", description: error.message,
                        });
                    }
                },
            });
            ...
            await updateApplication.mutateAsync(updateApplicationData, {
                onSuccess: () => {
                    ...
                    if (error instanceof TRPCClientError) {
                        toast({
                            variant: "destructive", title: "Update failed", description: error.message,
                        });
                    }
                },
            });
    ...
    const SCHOOL_OPTIONS = schoolsJson.map((entry, index) => ({
        value: entry.schoolName, label: entry.schoolName,
    }));
    return (<div className="flex w-3/5 justify-center">
            <Form {...form}>
                <form
                ...
                >
                    ...
                    <GenericCombobox
                        ...
                        options={[{value: "No", label: "I do have a team"}, {
                            value: "Yes",
                            label: "I do not have a team"
                        },]}
                        defaultOption={importedValues?.app.hasTeam ? {
                            value: importedValues.app.hasTeam, label: importedValues.app.hasTeam,
                        } : undefined}
                    />
                            ...
                            render={({field: {value, onChange, ...fieldProps}}) => (<FormItem>
                                    ...
                                    <FormControl>
                                        <Input
                                            ...
                                            className="border"
                                            onChange={(event) => {
                                                setValue("resumeFile", event.target.files ? event.target.files[0]! : null,);
                                            }}
                                        />
                                    </FormControl>
                                    <FormMessage/>
                    ...
                    <div className="pt-4">
                        <FormField
                            ...
                            render={({field}) => (<FormItem>
                                    ...
                                    <FormControl>
                                        <Input
                                            placeholder="308 Negra Arroyo Lane, Albuquerque, New Mexico 87104" {...field} />
                                    </FormControl>
                                    <FormMessage/>
                                </FormItem>)}
                        />
                    ...
                    <div className="pt-4">
                        <FormField
                            ...
                            render={({field}) => (<FormItem>
                                    ...
                                    <FormControl>
                                        <Input defaultValue={importedValues?.app.dietaryRestriction ?? ""}

                                               placeholder="Rock only diet." {...field} />
                                    </FormControl>
                                    <FormMessage/>
                                </FormItem>)}
                        />
                    ...
                    <div className="pt-4">
                        <FormField
                            ...
                            render={({field}) => (<FormItem>
                                    ...
                                    <FormControl>
                                        <Input placeholder="I love drywall!"
                    ...
                    <div className="flex items-center space-x-2 pt-4">
                        <FormField
                            ...
                            render={({field}) => (<FormItem>
                                    <FormLabel className="text-xl">I have read and accept the Liability
                                        Waiver: </FormLabel>
                                    <FormControl>
                                        <Checkbox
                                            checked={field.value}
